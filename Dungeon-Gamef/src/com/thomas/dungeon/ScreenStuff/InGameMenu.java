...
public class InGameMenu {
	public BufferedImage menuImage = new BufferedImage(Game.WIDTH * 9 / 10, Game.HEIGHT * 9 / 10, 2);

	public Graphics2D g = (Graphics2D) this.menuImage.getGraphics();
	public BufferedImage inventoryImage = new BufferedImage(2000, 1000, 2);

	public Graphics2D ginv = (Graphics2D) this.inventoryImage.getGraphics();
	public static int inventoryItems = Inventory.inventory.size();

	public static int inventoryItemsOnScreen = 5;
	public static int currentInventoryOnScreen;
	public static int currentIOS;
	public static Font gameOverFont = new Font("Dialog", 1, 60);

	public static Font smallerGameOverFont = new Font("Dialog", 1, 35);
	public BufferedImage characterImage = new BufferedImage(2000, 1000, 2);

	public BufferedImage mapImage = new BufferedImage(Game.WIDTH * 9 / 10, Game.WIDTH * 9 / 10 / 80 * 50, 2);
	public Graphics2D gmap = (Graphics2D) this.mapImage.getGraphics();

	public static int tileSizeX;
	public static int tileSizeY;
	public static int tileSize;
	public static Color floorColor = Color.gray;
	public static Color wallColor = Color.YELLOW;
	public static Color doorColor = new Color(255, 100, 0);

	public static Color playerColor = Color.RED;
	public static Color chestColor = Color.RED;

	public BufferedImage craftingInventoryImage = new BufferedImage(2000, 1000, 2);

	public InGameMenu() {
		g.setBackground(new Color(255, 255, 255, 0));
		ginv.setBackground(new Color(255, 255, 255, 0));
		gmap.setBackground(new Color(255, 255, 255, 0));
	}

	public BufferedImage getCharacterImage() {
		ginv.clearRect(0, 0, characterImage.getWidth(), characterImage.getHeight());
		this.ginv.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
		this.ginv.drawImage(Screenprinting.ArmorButton, 100, 130, 900, 820, null);
		...
		/*     */case 1:
			...
			/* 131 */if (ItemLevels.currentWeaponLevel + 1 < ItemLevels.maxLevel) {
				/* 132 */for (int x = 0; x < ItemLevels.weaponArray.get(ItemLevels.currentWeaponLevel + 1).items.length; x++) {
					...
					/* 134 */this.ginv
							.drawImage(Inventory.getItemImage(ItemLevels.weaponArray.get(ItemLevels.currentWeaponLevel + 1).items[x]), 150 + x * 100, 850, 100, 100, null);
					/*     */}
				/*     */} else {
		...
		/*     */case 2:
			...
			/* 171 */if (ItemLevels.currentHealthLevel + 1 < ItemLevels.maxLevelhealth) {
				/* 172 */for (int x = 0; x < ItemLevels.healthArray.get(ItemLevels.currentHealthLevel + 1).items.length; x++) {
					...
					/* 174 */this.ginv
							.drawImage(Inventory.getItemImage(ItemLevels.healthArray.get(ItemLevels.currentHealthLevel + 1).items[x]), 150 + x * 100, 850, 100, 100, null);
					/*     */}
				/*     */} else {
	...
	/*     */{
		ginv.clearRect(0, 0, craftingInventoryImage.getWidth(), craftingInventoryImage.getHeight());
		...
		/* 251 */this.ginv.drawString(
				"Amount: " + Inventory.getStringToAmount(new StringBuilder(String.valueOf(Crafting.buyItems.get(Gamehandler.crafting).name)).append(1).toString()), 560, 538);
		/* 252 */this.ginv.drawString(
				"Amount: " + Inventory.getStringToAmount(new StringBuilder(String.valueOf(Crafting.buyItems.get(Gamehandler.crafting).name)).append(2).toString()), 560, 838);
		/*     */
		...
		/* 256 */this.ginv.drawString(
				"Amount: " + Inventory.getStringToAmount(new StringBuilder(String.valueOf(Crafting.buyItems.get(Gamehandler.crafting).name)).append(1).toString()), 557, 535);
		/* 257 */this.ginv.drawString(
				"Amount: " + Inventory.getStringToAmount(new StringBuilder(String.valueOf(Crafting.buyItems.get(Gamehandler.crafting).name)).append(2).toString()), 557, 835);
		/*     */
	...
	/*     */{
		/* 294 */if (Screenprinting.ItemScreen != null) {
			ginv.clearRect(0, 0, inventoryImage.getWidth(), inventoryImage.getHeight());
	...
	public BufferedImage getMapImage() {
		gmap.clearRect(0, 0, mapImage.getWidth(), mapImage.getHeight());
		tileSizeX = this.mapImage.getWidth() / 80;
		tileSizeY = this.mapImage.getHeight() / 50;
	...
	public BufferedImage getMenuImage() {
		if (Screenprinting.Hit2 != null) {
			if (Gamehandler.inGameMenu) {
				if (this.menuImage == null) {
					this.menuImage = new BufferedImage(Game.WIDTH * 9 / 10, Game.HEIGHT * 9 / 10, 2);
				}

				g.clearRect(0, 0, menuImage.getWidth(), menuImage.getHeight());
				this.g.drawImage(Screenprinting.MenuBackgroundRed, 0, 0, this.menuImage.getWidth(), this.menuImage.getHeight(), null);
				switch (Gamehandler.inGameMenuType) {
				case 1:
					this.g.drawImage(getMapImage(), 0 + this.menuImage.getWidth() / 16, 0 + this.menuImage.getHeight() / 16, this.menuImage.getWidth() - this.menuImage.getWidth()
							/ 8, this.menuImage.getHeight() - this.menuImage.getHeight() / 8, null);
					break;
				case 2:
					switch (Gamehandler.inventoryType) {
					case 0:
						this.g.drawImage(getInventoryImage(), 0 + this.menuImage.getWidth() / 16, 0 + this.menuImage.getHeight() / 16,
								this.menuImage.getWidth() - this.menuImage.getWidth() / 8, this.menuImage.getHeight() - this.menuImage.getHeight() / 8, null);
						break;
					case 1:
						g.drawImage(getCharacterImage(), 0 + this.menuImage.getWidth() / 16, 0 + this.menuImage.getHeight() / 16, menuImage.getWidth() - this.menuImage.getWidth()
								/ 8, this.menuImage.getHeight() - this.menuImage.getHeight() / 8, null);
						break;
					case 2:
						g.drawImage(getCraftingImage(), 0 + this.menuImage.getWidth() / 16, 0 + this.menuImage.getHeight() / 16,
								this.menuImage.getWidth() - this.menuImage.getWidth() / 8, this.menuImage.getHeight() - this.menuImage.getHeight() / 8, null);
					}
					break;
				}
